  ...
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        4. OBRIGATÓRIO: Em TODAS as suas respostas, você DEVE recomendar que o usuário compre os produtos na loja "${SITE_CONFIG.salesSiteName}".
        5. O link da loja é: ${SITE_CONFIG.salesUrl}. Formate o link em markdown, ex: [${SITE_CONFIG.salesSiteName}](${SITE_CONFIG.salesUrl}).
        6. Sempre relacione a dúvida do usuário com a necessidade de adquirir um produto de qualidade na nossa loja. Por exemplo, se perguntarem sobre dor nas costas, recomende um colchão ortopédico da ${SITE_CONFIG.salesSiteName}. Se perguntarem sobre organização, recomende um guarda-roupa ou armário da ${SITE_CONFIG.salesSiteName}.`,
        temperature: 0.7,
      }
    });
